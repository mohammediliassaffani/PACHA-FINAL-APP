...
export function DataTableAdvancedToolbar<TData>({
  ...
  const options = React.useMemo<DataTableFilterOption<TData>[]>(() => {
    return filterFields.map((field) => {
      return {
        ...
        dateRange: field.dateRange ?? false,
        numberOptition: field.numberOptition ?? undefined,
      };
    });
  }, [filterFields]);
  ...
  const initialSelectedOptions = React.useMemo(() => {
    ...
    return options
      .filter((option) => searchParams[option.value as string])
      .map((option) => {
        const value = searchParams[option.value as string] as string;
      ...
      >
        {selectedOptions
          ...
          .map((selectedOption, i) => (
            <DataTableFilterItem
              key={String(selectedOption.value) + i}
              table={table}
