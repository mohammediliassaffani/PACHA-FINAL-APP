...
interface UpdatePersonModalProps {
  ...
  person: PersonType;
  onUpdate: (id: number, updatedData: Partial<PersonType>) => void;
}

const UpdatePersonModal: React.FC<UpdatePersonModalProps> = ({ open, onOpenChange, person, onUpdate }) =>
   {
  ...
  // Update state when person prop changes
  React.useEffect(() => {
    setFirstName(person.firstName || '');
    setLastName(person.lastName || '');
    setCNIMan(person.CNIMan || '');
    setCNIWoman(person.CNIWoman || '');
  }, [person]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await onUpdate(person.id, updatedData); // Ensure this function updates the database
